...
#endif
    float result= _pModel->findNearest(sample, 2, results, neighborResponses,dists);
...
void RecogniserKNearest::recognise(const cv::Mat &image, const vector<vector<cv::Rect> > &lines,
		string &text, bool black_on_white)
{
    Recogniser::recognise(image, lines, text, black_on_white);
}
