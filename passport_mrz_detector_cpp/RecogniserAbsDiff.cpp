...
{
    /*
    std::cout<< charset.c_str() << "--------" << input_dir.c_str() << std::endl;
    std::cout<< *charset.begin() << "--------" << *charset.end() << std::endl;
    ...
    */
    ...
    if ((dir = opendir (input_dir.c_str())) != NULL) {
        ...
        while ((ent = readdir (dir)) != NULL) {
            pch = strpbrk (ent->d_name, "png");
            if(pch != NULL)
            {
                std::vector<std::string> strsplit{explode(ent->d_name, '.')};
                //std::cout<<  "--------" << strsplit.at(0) << std::endl;
                file_names.push_back(strsplit.at(0));
                //file_name += ".png";
                //char_images.push_back(cv::imread(file_name, CV_LOAD_IMAGE_GRAYSCALE));
            }
            //printf ("%s\n", ent->d_name);
...
void RecogniserAbsDiff::recognise(const cv::Mat& img, const std::vector<std::vector<cv::Rect> > &lines, std::string &text)
{
	std::for_each(lines.begin(), lines.end(), [this, img, &text](const std::vector<cv::Rect> &line) {
		std::for_each(line.begin(), line.end(), [this, img, &text](const cv::Rect &char_rect) {
			...
			text += recognise(char_image, true, subsets::ALL);
		});
	});
}
