...
void RecogniserTesseract::set_image_bmp(const void *bmp_data)
{
    ...
    const unsigned char *pixel_data = (const unsigned char *)((*ptr) + hdr->bfOffBits);
...
void RecogniserTesseract::recognise(const cv::Mat &image, const vector<vector<cv::Rect> > &lines,
